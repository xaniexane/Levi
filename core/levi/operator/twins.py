...
class TwinArrival:
    ...
    def banner(self) -> str:
        template = HIGH_STAKES_BANNER if self.stakes == "high" else FORCED_BANNER
...
def run_twin_turn(
    ...
    if mode == "always":
        ...
        fork_stakes = stakes
        fork_reason = (
            "stakes assessed high" if stakes == "high" else "force_twin set in context"
        )
    else:
        fork = False
        fork_reason = ""
    ...
    marker = (
        "VERIFIED" if verified else "no clear verdict (no VERIFIED/CHALLENGED marker)"
    )
...
def record_pick(
    ...
    if registry_or_store is not None:
        if hasattr(registry_or_store, "add") and hasattr(registry_or_store, "list"):
            ...
            # fatal.
            try:
                known = {info["name"] for info in registry_or_store.list_operators()}
                if winner_name not in known:
                    raise ValueError(
                        f"record_pick: {winner_name!r} is not a registered operator"
                    )
            except ValueError:
                raise
            except Exception:
