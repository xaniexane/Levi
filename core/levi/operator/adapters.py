...
- :class:`DynastyOperatorAdapter` — wraps a wave agent. A native
  ``DynastyAgent`` rides as ``kind="native"``; an
  ``IntegratedAgent`` (foreign intelligence) rides as
  ``kind="ai"`` with ``is_foreign=True`` — the
  integrated-intelligence leash (scoped tools, declared origin,
  output screening) applies through this adapter. The dynasty
  package itself is NEVER edited; wave-agent specifics stay in
  dynasty records.
...
class ChatProviderAdapter(Operator):
    ...
    def step(
        ...
        calls = [
            OperatorToolCall(
                id=c.id, name=c.name, arguments=dict(c.arguments or {})
            )
...
def chat_provider_operator(provider: ChatProvider, *, kind: str = SI) -> ChatProviderAdapter:
...
# ---------------------------------------------------------------------------
# DynastyOperatorAdapter — wave agents behind the contract (wrap only)
# ---------------------------------------------------------------------------


class DynastyOperatorAdapter(Operator):
    """A dynasty wave agent behind the Operator contract.

    The dynasty package is NEVER edited here — this only wraps it. A
    native ``DynastyAgent`` rides as ``kind="native"``; an
    ``IntegratedAgent`` (a foreign intelligence on the
    integrated-intelligence leash) rides as ``kind="ai"`` with
    ``is_foreign=True``: scoped tools only, declared origin, and
    instruction-override screening on its output. Wave-agent
    specifics stay in dynasty records; nothing dynasty-internal is
    exposed through this adapter beyond the sealed act() result.
    """

    def __init__(self, agent: Any, *, kind: Optional[str] = None) -> None:
        from levi.dynasty import adapter as _da
        from levi.dynasty import dna as _dna

        if isinstance(agent, _da.IntegratedAgent):
            resolved_kind = AI
            foreign = True
        elif isinstance(agent, _dna.DynastyAgent):
            resolved_kind = NATIVE
            foreign = False
        else:
            raise OperatorContractError(
                "DynastyOperatorAdapter needs a DynastyAgent, got "
                f"{type(agent).__name__}"
            )
        self._agent = agent
        self._integrated_cls = _da.IntegratedAgent
        self.name = getattr(agent, "agent_id", None) or "dynasty-agent"
        self.kind = kind or resolved_kind
        self.is_foreign = foreign
        self.version = "0.1.0"
        origin = getattr(agent, "origin", "dynasty")
        self.lineage = f"dynasty:{origin}:{self.name}"

    def capabilities(self) -> OperatorCapabilities:
        # The agent's act() handles tasks internally; the contract
        # surface declares no direct tool emission. Scoped wares stay
        # dynasty-internal (the integrated leash enforces them there).
        return OperatorCapabilities(
            tools=(),
            streaming=False,
            memory_access=False,
            context_window=4096,
            tool_use_loop=False,
            notes=f"adapter over wave agent {self.name!r} (act() path)",
        )

    def health(self) -> OperatorHealth:
        return OperatorHealth(ok=True, note=f"wave agent {self.name!r} enrolled")
    ...
    def step(
        ...
        try:
            ...
            return OperatorResult(
                ...
                latency_ms=(time.perf_counter() - t0) * 1000.0,
                finish_reason="error",
                error=f"wave agent {self.name!r} raised {type(exc).__name__}: {exc}",
            )
        text = str(sealed)
        if self.is_foreign:
            try:
                text = screen_foreign_text(text, operator_name=self.name)
