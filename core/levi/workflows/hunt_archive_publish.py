...
def _fail(steps: List[Dict[str, Any]], step: Dict[str, Any],
          exc: Exception, artifacts: Dict[str, Any]) -> Dict[str, Any]:
...
def run(home=None, findings: Optional[List[Dict[str, Any]]] = None,
        wave_id: Optional[str] = None, author: str = "levi-workflows",
        **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        wave = wave_id or ("wave-%s" % datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
        record_ids = [r.id for r in records]
        finish_step(step, True, {"records": len(records), "wave_id": wave,
    ...
    try:
        ...
        finish_step(step, True, {"queue": str(queue_path),
                                 "records": len(records)})
    ...
    try:
        with workflow_env(levi_home):
            queued = _hunt.queue_for_build(
                records, wave, home=str(levi_home.parent)
            )
            queued_items = _hunt.read_build_queue(home=str(levi_home.parent))
        queued_ids = {i.get("record_id") for i in queued_items
                      if i.get("wave_id") == wave}
        ...
        for rec in records:
            if rec.id not in queued_ids:
                ...
                jobs.append({
                    ...
                    "title": rec.title,
                    "rating": rec.rating,
                    "note": ("no cost-bearing way allowed — manufacture a "
    ...
    try:
        ...
        record = {
            ...
            "description": "Hunt collection %s: %d archived finds" % (wave, len(record_ids)),
