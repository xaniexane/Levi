...
def _fmt(e) -> str:
    return (
        f"[{e.id}] {e.memory_type.value} importance={e.importance:.2f} "
        f"tags={','.join(e.tags)}\n  {e.content[:300]}"
    )
...
def cmd_add(a) -> int:
    ...
    try:
        ...
        return 2
    try:
        e = _store(a.data_dir).add(
            mtype, a.text, importance=a.importance, source=a.source, tags=tags or None
        )
...
def cmd_search(a) -> int:
    ...
    print(
        "no matches."
        if not hits
        else "\n".join(_fmt(e) + f"  [score {s:.3f}]" for e, s, _w in hits)
    )
    return 0
...
def cmd_receipts(a) -> int:
    ...
    for r in receipts:
        print(
            "[%s] %s src=%s created=%s importance=%.2f%s"
            % (
                r["id"],
                r["type"],
                r["source"],
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="levi.memory", description="LEVI memory store — local-first"
    )
    ap.add_argument("--data-dir", default=default_dir)
    sub = ap.add_subparsers(dest="cmd", required=True)

    p = sub.add_parser("add", help="add a memory entry")
    ...
    p = sub.add_parser("search", help="retrieve for a query")
    ...
    p = sub.add_parser(
        "receipts",
        help="provenance ledger: what LEVI remembers, with source and timestamp",
    )
    ...
    p.set_defaults(func=_info, info="explain")
