...
def _parser() -> argparse.ArgumentParser:
    ...
    s.add_argument(
        "--kind-filter", default="", help="comma-separated kinds, e.g. 'course,book'"
    )
    ...
    s.add_argument(
        "--weights", default=None, help='"goal=0.5,content=0.3,quality=0.2,recency=0"'
    )
    s.add_argument("--half-life", type=float, default=DEFAULT_HALF_LIFE_DAYS)
    s.add_argument(
        "--exclude-rated", action="store_true", help="skip items you already rated"
    )
...
def cmd_add_item(args) -> int:
    ...
    try:
        item = Item(
            id=item_id, title=args.title, kind=args.kind, topics=topics, text=args.text
        )
    except ValueError as exc:
...
def cmd_rate(args) -> int:
    ...
    print(
        "rated %r %.1f/5 (explicit — this is the only taste signal)"
        % (args.item_id, value)
    )
    return 0
...
def cmd_recommend(args) -> int:
    ...
    try:
        recs = recommend(
            ...
            mmr_lambda=args.diverse,
        )
    except ValueError as exc:
        print("bad options: %s" % exc, file=sys.stderr)
        return 2
    print(
        "weights: %s   goals: %s   diversity λ=%.2f"
        % (
    ...
    for i, rec in enumerate(recs, 1):
        if args.explain:
            ...
            print("#%d [%.4f] %s [%s]" % (i, rec.total, rec.item.title, rec.item.kind))
    return 0
...
def cmd_list(args) -> int:
    if args.what == "items":
        ...
        for item_id, vals in sorted(load_ratings().items()):
            print("%-24s %s" % (item_id, ", ".join("%.1f" % v for v in vals)))
    return 0
...
def cmd_export(args) -> int:
    ...
    print(
        "exported %d item(s), %d goal(s) to %s (open format, no lock-in)"
