...
def run_integration_audit() -> str:
    ...
    def demand_income():
        ...
        return "demand_signal_id" in ServicePlan.__dataclass_fields__, "plan carries signal id"
    ...
    def hitl_rail_domain():
        ...
        return "opportunity_rail" in ALWAYS_HITL or "pricing" in ALWAYS_HITL, "HITL covers rail domains"
    ...
    def symbiosis_pairs():
        ...
        return hasattr(s, "value_check") or hasattr(s, "format_status"), "symbiosis module"
    ...
    def chat_companion():
        ...
        return len(out) > 30 and ("LEVI" in out or "companion" in out.lower()), f"len={len(out)}"
    ...
    def full_cloud_model():
        ...
        return snap.get("seal") == "L.W.P." and "expand" in m.caps.organs, (
            "words=%s rank=%s organs=%d integrations=%d" % (
                snap.get("words"), snap.get("rank"), len(m.caps.organs), len(m.caps.integrations)
            )
        )
