...
def run_enterprise_checklist() -> List[Check]:
    ...
    def hitl():
        ...
        domains = ALWAYS_HITL if isinstance(ALWAYS_HITL, (set, list, tuple)) else list(ALWAYS_HITL)
        g = HITLGate()
        return len(domains) >= 3, f"domains={len(domains)} pending={len(g.list_pending())}"

    def crisis():
        from levi.ei.offline_companion import synthesize
        t = synthesize("I'm panicking and falling apart")
        return len(t) > 40 and ("steady" in t.lower() or "urgent" in t.lower()), f"len={len(t)}"
    ...
    def provenance():
        ...
        r = provenance_report() if callable(provenance_report) else str(provenance_report)
    ...
    def no_silent_contact():
        ...
        return "customer" in s or "contact" in s or "opportunity_rail" in s, "HITL covers consequential contact domains"
