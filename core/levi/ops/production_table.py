...
ENGINES: List[Tuple[str, str, str, str]] = [
    ...
    ("prehistoric", "HITL approval cards", "Human on consequences (silence≠yes)", "prod"),
    ("prehistoric", "Emergency stop", "Hard kill switch on kernel", "prod"),
    # Modern industry patterns (original implementations)
    ("modern", "Model relay chain", "Local Ollama → offline synth; cloud config-ready", "prod"),
    ...
    # LEVI-unique
    ("levi-unique", "L.W.P. physics", "Cascade, spiral, governor, Mirror coils", "prod"),
    ("levi-unique", "L.W.P. Model Engine", "Offline SSA literary spine; REIM/void/ROM; UI+CLI", "prod"),
    ...
    ("levi-unique", "Organism interpenetration", "One bloodstream, not side apps", "prod"),
    ("levi-unique", "Charter (bound soul)", "Editable identity; cannot kill HITL/crisis", "prod"),
    ("levi-unique", "Nervous system + tone", "Stress/bond persona selection, crisis mute wit", "prod"),
    ("levi-unique", "Hardwired core logic", "Alchemy, life equation, life chess, radical honesty", "prod"),
    ("levi-unique", "Symbiosis pairs", "Demand↔Income; every asset needs other half", "prod"),
    ("levi-unique", "Story fabric genres", "Cascade beats + social_media + abridged_series", "prod"),
    ("levi-unique", "Echo / Mandella organs", "Parallel path + stake organs", "prod"),
    ("levi-unique", "Factory under L.W.P.", "Software Factory as DNA not separate product", "prod"),
    ("levi-unique", "Ugly truth / constructive discomfort", "Director not pleaser", "prod"),
    ("levi-unique", "Monotropism tracker", "Tunnel depth under focus load", "prod"),
    ("modern", "Pollinations stills", "Optional image URL generation for beats", "prod"),
    ...
    ("levi-unique", "Continuity Shelf", "Cross-organ resume pointers local JSON", "prod"),
...
def production_table() -> str:
    ...
    lines.append("")
    lines.append("Continuous regime: scripts/fast_gate.sh on every change · levi go daily · HITL on consequences")
    lines.append("Deploy regime: local-first package only — no silent cloud deploy. Private git when scopes allow.")
    lines.append("")
    lines.append("=== DEFERRED ENGINES (clarified) ===")
    lines.append("Fat interactive UI     — not required for complete-below-enterprise; CLI+ops is the control plane")
    lines.append("Live multi-cloud billing — would force paid path; violates free-first / local-first for core")
    lines.append("Glyph compressed memory — optimization layer; corpus+brain already operable; compress later")
    lines.append("These are intentional non-goals for v1 complete, not abandoned architecture.")
    return "\n".join(lines)
