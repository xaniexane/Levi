...
def perfection_report() -> str:
    ...
    def organs():
        ...
        return len(ORGANS) >= 28 and len(
            BLOODSTREAM
        ) >= 30, f"organs={len(ORGANS)} bonds={len(BLOODSTREAM)}"
    ...
    def ui():
        ...
        s = find_static()
        return (s / "levi-ops.html").exists() and (s / "lwp-model.html").exists(), str(
            s
        )
    ...
    lines.append(
        "Deferred by design: fat multi-tenant UI, live multi-cloud billing, glyph compress."
    )
    return "\n".join(lines)
