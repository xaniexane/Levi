...
def cmd_audit(args) -> int:
    if args.manifest:
        ...
        if unknown:
            print("unknown manifest fields: %s" % ", ".join(unknown),
                  file=sys.stderr)
            return 2
        manifest = GenerosityManifest(**{k: v for k, v in data.items()
                                         if k in known})
    else:
...
def cmd_showcase() -> int:
    ...
    for key, result in results.items():
        ...
        if result.passed:
            ...
            ok = False
    print("showcase: %s" % ("all four giants fail as expected"
                            if ok else "CHARTER IS TOOTHLESS"))
...
def main(argv=None) -> int:
    ...
    pa.add_argument("--manifest", default=None,
                    help="JSON file with GenerosityManifest fields")
