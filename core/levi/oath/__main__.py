...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    print(
        "next: oath key gen --uid 'Owner <you@example.com>', then set owner.json fingerprint,"
    )
...
def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    _check("oath home exists", oath_home().is_dir(), "run: python -m levi.oath init")
    ...
    try:
        owner_fp = (
            (
                json.loads(OWNER_FILE().read_text(encoding="utf-8")).get("fingerprint")
                or ""
            )
            .upper()
            .replace(" ", "")
        ...
        pass
    _check(
        "owner.json fingerprint set", bool(owner_fp), "set fingerprint in owner.json"
    )

    keys = list_keys() if gpg_available() else []
    has_owner_key = any(k.fingerprint == owner_fp for k in keys) if owner_fp else False
    _check(
        "owner key present in oath keyring",
        has_owner_key,
        "oath key gen / oath key import",
    )
    ...
    _check(
        "mail.json configured (imap or maildir usable)",
        bool(cfg.imap_host) or MAILDIR().is_dir(),
        "fill in mail.json or drop mail into maildir/new",
    )
...
def cmd_key(args: argparse.Namespace) -> int:
    ...
    if args.key_cmd == "gen":
        ...
        print(
            "pin this fingerprint in owner.json (owner) or the contact (oath contact pin)"
        )
        return 0
...
def cmd_contact(args: argparse.Namespace) -> int:
    ...
    if args.contact_cmd == "add":
        contact = Contact(
            name=args.name,
            email=args.email or "",
            trust_floor=args.floor,
            tier_ceiling=args.ceiling,
    ...
    if args.contact_cmd == "list":
        for name, contact in sorted(book.contacts.items()):
            grants = ", ".join(
                f"{c}:{''.join(l)}" for c, l in sorted(contact.grants.items())
            )
            print(
...
def cmd_inbox(args: argparse.Namespace) -> int:
    ...
    missions = poll(
        load_book(),
        use_imap=args.imap,
        use_maildir=not args.imap,
        config=load_mail_config(),
    )
    for key, mission in missions:
        print(
            f"{key}: from={mission.from_address} trust={mission.trust} "
...
def cmd_daemon(args: argparse.Namespace) -> int:
    ...
    daemon = Daemon(
        config=load_mail_config(), poll_interval=args.interval, dry_run=args.dry_run
    )
    if args.once:
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    if args.audit_cmd == "verify":
        ...
        try:
            ...
            return 1
        print(
            f"chain ok: {chain['entries']} entries, head {chain['head'][:16]}…; "
    ...
    if args.audit_cmd == "tail":
        ...
        for entry in entries[-args.n :]:
            ...
            print(
                f"{entry['seq']:>5} {entry['ts']} {payload.get('event', '?')} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    q.add_argument(
        "--ceiling", default="write", choices=["read", "write", "execute", "dangerous"]
    )
    ...
    q.add_argument("name")
    q.add_argument(
        "--tier", default="read", choices=["read", "write", "execute", "dangerous"]
    )
    ...
    q.add_argument(
        "--expires", default="", help="ISO-8601 expiry, e.g. 2027-09-15T00:00:00Z"
    )
