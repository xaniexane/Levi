...
def build_questions(
    store, per_chunk: int = 1, max_questions: int = 50
) -> List[EvalQuestion]:
    ...
    """
    entries = [
        e for e in (store.list(limit=100000) or []) if (e.metadata or {}).get("rag")
    ]
    ...
    for entry in sorted(entries, key=lambda e: e.id):
        ...
        if not kws:
            ...
            content_terms = sorted(
                set(t for t in tokenize(entry.content or "") if len(t) > 5),
...
def evaluate(
    ...
    questions = build_questions(store, per_chunk=per_chunk, max_questions=max_questions)
    rows: List[EvalRow] = []
    for q in questions:
    ...
    return {
        ...
        "mean_rank": (sum(r.rank for r in rows if r.rank) / hits_n) if hits_n else None,
...
def print_report(report: Dict) -> str:
    ...
    lines = [
        "=== LEVI retrieval eval ===",
        "method=%s k=%d questions=%d"
        % (report["method"], report["k"], report["total"]),
        "hit-rate@%d: %.1f%% (%d/%d)"
        % (report["k"], 100 * report["hit_rate"], report["hits"], report["total"]),
