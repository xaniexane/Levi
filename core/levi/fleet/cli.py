...
def register_fleet(sub) -> None:
    ...
    run_p.add_argument(
        "--max-minutes", type=float, default=10.0, help="Wall-clock budget in minutes"
    )
    ...
    run_p.add_argument(
        "--model", action="store_true", help="Use the agentic loop for decomposition"
    )
...
def _cmd_categories() -> int:
    ...
    for c in list_categories():
        ...
        print(f"{c.name:16} {c.cost_class:8} {len(c.tools):2} tools{stub}")
        print(f"    {c.role}")
    if problems:
        print("\nRegistry problems:")
        for p in problems:
...
def _cmd_run(args) -> int:
    ...
    print(
        f"Spent: {spent['agents']} agents, {spent['tool_calls']} tool "
