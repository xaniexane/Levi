...
def structural_check(node: Dict[str, Any], result: Dict[str, Any],
                     blackboard: Any) -> Verification:
    ...
    if len(summary) < 8:
        return Verification(False, "empty or trivial result summary",
                            "structural")
    for art in result.get("artifacts") or []:
        name = art.get("name") if isinstance(art, dict) else art
        if name and blackboard.get(f"artifact:{name}") is None:
            return Verification(False, f"claimed artifact {name!r} missing "
                                "from blackboard", "structural")
    return Verification(True, "structural checks passed", "structural")


def semantic_check(node: Dict[str, Any], result: Dict[str, Any],
                   provider: Any = None) -> Verification:
    ...
    try:
        ...
        return Verification(True, "semantic check unavailable (no loop)",
                            "structural")
    prompt = (
    ...
    try:
        transcript = run_subtask(prompt, provider=provider or "local",
                                 max_steps=3,
                                 system_prompt="You verify. One line only.")
        text = (getattr(transcript, "final", "") or "").strip()
    except Exception as exc:
        return Verification(True, f"semantic check errored ({exc}); "
    ...
    return Verification(True, "semantic check inconclusive; structural only",
                        "structural")
...
def verify_node(node: Dict[str, Any], result: Dict[str, Any],
                blackboard: Any, *,
