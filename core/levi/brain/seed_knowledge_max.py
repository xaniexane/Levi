...
_AZ_EXTRA = {
    "A": [
        "Algorithms trade time and space under constraints.",
        "Agency requires both capability and permission.",
    ],
    "B": [
        "Bandwidth is finite; prioritization is inevitable.",
        "Boundaries make systems legible.",
    ],
    "C": [
        "Contracts allocate risk; ambiguity allocates lawsuits.",
        "Calibration beats confidence theater.",
    ],
    "D": [
        "Debt is deferred work with interest in complexity.",
        "Diagnostics before treatment.",
    ],
    "E": [
        "Entropy increases without work against it.",
        "Evidence updates beliefs; identity should not block updates.",
    ],
    "F": [
        "Friction can be protective or wasteful; diagnose which.",
        "Failure modes should be rehearsed.",
    ],
    "G": [
        "Governance is decision rights plus accountability.",
        "Graceful degradation preserves core function.",
    ],
    "H": [
        "Habit design beats willpower narratives.",
        "Horizon scans need explicit uncertainty.",
    ],
    "I": ["Incentives dominate exhortation.", "Interfaces are promises."],
    "J": [
        "Judgment under uncertainty is a skill, not a vibe.",
        "Jurisdiction limits what tools can impose.",
    ],
    "K": [
        "Knowledge compounds when shared with provenance.",
        "Key management is security's boring center.",
    ],
    ...
    "O": [
        "Observability closes the loop on reality.",
        "Ownership must be singular for critical paths.",
    ],
    "P": [
        "Provenance makes claims checkable.",
        "Privacy is contextual integrity, not secrecy alone.",
    ],
    "Q": [
        "Quality is absence of surprise in the field.",
        "Queues need bounds and policies.",
    ],
    "R": [
        "Reversibility preserves option value.",
        "Reliability is a probability, not a slogan.",
    ],
    "S": ["Simplicity is a hard-won artifact.", "Stewardship includes exit plans."],
    "T": [
        "Trade-offs are the real design document.",
        "Trust is earned under verification.",
    ],
    "U": [
        "Uncertainty must be represented, not hidden.",
        "Usability is cognitive load engineering.",
    ],
    "V": [
        "Validation checks the problem; verification checks the build.",
        "Versioning makes change survivable.",
    ],
    "W": [
        "Work-in-progress limits reduce thrash.",
        "Write-downs of bad ideas free capacity.",
    ],
    "X": [
        "X-risk needs sober priors, not performance panic.",
        "Unknowns require labeled hedges.",
    ],
    "Y": [
        "Yield management applies beyond airlines: scarce slots.",
        "Youth skill windows reward deliberate practice.",
    ],
    "Z": [
        "Zero-trust assumes hostile networks.",
        "Zoom out to check local fixes for global harm.",
    ],
}
...
def _build() -> List[Tuple[str, str, Tuple[str, ...]]]:
    ...
    for letter, lines in _AZ_EXTRA.items():
        for line in lines:
            raw.append(
                (f"Max {letter}: {line}", "OBSERVED", ("max", "az", letter.lower()))
            )
    ...
    for v in verbs:
        for n in nouns:
            raw.append(
                (
                    f"Operator — {v} {n} with explicit ownership and a failure plan.",
                    "INFERENCE",
                    ("max", "operator"),
                )
            )
    ...
    constraints = [
        ...
        "with scarce attention",
        "with legacy coupling",
    ]
    domains2 = [
        "architecture",
        "incident response",
    ...
    for c in constraints:
        for d in domains2:
            raw.append(
                (
                    f"Practice — Handle {d} {c}; name the trade-off and the exit ramp.",
                    "INFERENCE",
                    ("max", "practice"),
                )
            )
    ...
    for g in genres:
        for b in beats:
            raw.append(
                (
                    f"Story craft — In {g}, the {b} beat must respect scar law and sensory specificity.",
                    "OBSERVED",
                    ("max", "story", g),
                )
            )
    ...
    for i in range(1, 101):
        raw.append(
            (
                f"Principle {i:03d} — Make the next move reversible unless the cost of delay exceeds the cost of error.",
                "INFERENCE",
                ("max", "principle"),
            )
        )
    for i in range(1, 101):
        raw.append(
            (
                f"Check {i:03d} — Before consequential action: owner, risk, HITL gate, rollback, evidence label.",
                "OBSERVED",
                ("max", "check"),
            )
        )
    ...
    for pe in personas:
        for mo in modes:
            raw.append(
                (
                    f"Persona×mode — {pe} under {mo}: route for fit; never for humiliation or crisis theater.",
                    "OBSERVED",
                    ("max", "persona", pe, mo),
                )
            )
    ...
    for phase in ["A", "B", "C"]:
        for topic in [
            ...
            raw.append(
                (
                    f"Phase {phase} — {topic}: local core remains authoritative; cloud stays optional wing.",
                    "OBSERVED",
                    ("max", "phase", phase.lower()),
                )
            )
    ...
    for pr in premiums:
        for ang in angles:
            raw.append(
                (
                    f"Premium {pr} / {ang} — design so offline SI remains useful without cloud dependency.",
                    "INFERENCE",
                    ("max", "premium", pr),
                )
            )
    ...
    for i in range(101, 401):
        raw.append(
            (
                f"Principle {i:03d} — Prefer explicit trade-offs over hidden defaults that bind the future.",
                "INFERENCE",
                ("max", "principle"),
            )
        )
    ...
    for i in range(1, 201):
        raw.append(
            (
                f"Evidence drill {i:03d} — Label the claim; name the weakest premise; seek disconfirming data.",
                "OBSERVED",
                ("max", "evidence"),
            )
        )
    ...
    for i in range(1, 301):
        raw.append(
            (
                f"Runbook {i:03d} — On alarm: verify signal, contain blast radius, communicate owner, preserve evidence.",
                "OBSERVED",
                ("max", "runbook"),
            )
        )
    for i in range(1, 301):
        raw.append(
            (
                f"Build note {i:03d} — Smallest vertical slice that proves the riskiest assumption.",
                "INFERENCE",
                ("max", "build"),
            )
        )
    for i in range(1, 201):
        raw.append(
            (
                f"Care note {i:03d} — Under distress: reduce options, lower wit, one concrete next step.",
                "OBSERVED",
                ("max", "care"),
            )
        )
