...
CATALOG: List[FreeIntegration] = [
    ...
    FreeIntegration(
        "imap_readonly",
        "hybrid",
        "Optional local mail triage drafts",
    ...
    FreeIntegration(
        "git_handoff",
        "modern",
        "Private git as continuity shelf",
        "git CLI user-owned",
    ...
    FreeIntegration(
        "mirror_cascade",
        "levi-unique",
        "Triple-coil reverse cross-check",
        "pure Python",
    ...
    FreeIntegration(
        "lwp_ssa",
        "levi-unique",
        "L.W.P. Model offline literary SSA",
    ...
    FreeIntegration(
        "symbiosis_value",
        "levi-unique",
        "Other-half + non-malicious value rules",
        "pure Python",
    ...
    FreeIntegration(
        "monotropism_physics",
        "levi-unique",
        "Attention tunnel interaction physics",
        "pure Python",
    ...
    FreeIntegration(
        "alchemy_core",
        "levi-unique",
        "Hardwired core logic (alchemy/equation/chess)",
        "pure Python",
...
def format_catalog() -> str:
    ...
    lines.append(
        "Core boots with zero paid APIs. Upsells never gate crisis/HITL/offline."
    )
    return "\n".join(lines)
...
def interpenetration_matrix() -> str:
    ...
    lines.append(
        "  weighting: declared combines_with = 1.0; symbiosis formal pair = 2.0;"
    )
    lines.append(
        "             same pair declared in both registries => weights add (3.0)"
    )
    dang = g.dangling_refs()
    lines.append(
        f"  dangling refs: {len(dang)}" + ("" if not dang else f" ({', '.join(dang)})")
    )
    lines.append(
    ...
    top = sorted(
        ((n, g.weighted_degree(n)) for n in g.nodes), key=lambda t: (-t[1], t[0])
    )[:5]
    for nid, w in top:
