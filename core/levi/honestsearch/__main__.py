...
def _parser() -> argparse.ArgumentParser:
    ...
    s.add_argument(
        "--explain", action="store_true", help="show per-signal contributions"
    )
    s.add_argument("--weights", default=None, help="override, e.g. text=0.8,link=0.2")
...
def _read_local(path: Path):
    ...
    if suffix in (".html", ".htm"):
        ...
        return (
            title_of(raw) or path.name,
            extract_text(raw, limit=20000),
...
def cmd_add(args) -> int:
    ...
    save_index(index)
    print(
        "added %d file(s); index now holds %d document(s) at %s"
...
def cmd_crawl(args) -> int:
    crawler = SiteCrawl(
        max_pages=args.max_pages, max_depth=args.max_depth, scope=args.scope
    )
    ...
    print(
        "crawled %d page(s) (%d refusals recorded, nothing fabricated)"
...
def cmd_query(args) -> int:
    ...
    results = search(index, " ".join(args.terms), weights=weights, top_k=args.top)
    print(
        "weights: %s   (unpersonalized: no profile enters ranking)"
        % ", ".join("%s=%.2f" % (k, v) for k, v in normalize_weights(weights).items())
    )
    if not results:
        print("no matches")
        return 1
    for i, res in enumerate(results, 1):
        if args.explain:
            ...
            print("#%d [%.4f] %s\n    %s" % (i, res.total, res.doc.title, res.doc.url))
    return 0
...
def cmd_weights(args) -> int:
    if args.action == "show" or not args.spec:
        ...
        print("current: %s" % ", ".join("%s=%.2f" % kv for kv in current.items()))
        print("default: %s" % ", ".join("%s=%.2f" % kv for kv in default.items()))
        return 0
    try:
        ...
        return 2
    print("weights set: %s" % ", ".join("%s=%.2f" % kv for kv in cleaned.items()))
    return 0
...
def cmd_export(args) -> int:
    ...
    print("exported %d document(s) to %s (open format, no lock-in)" % (len(index), out))
    return 0
