"""
Phase A / B / C map — fused into Vyve LEVI as first-class organism layers.
...
class PhaseMap:
    ...
    def all(self) -> List[PhaseDef]:
        return [self.phases[k] for k in ("A", "B", "C")]

    def get(self, pid: str) -> Optional[PhaseDef]:
        return self.phases.get(pid.upper())

    def status_block(self) -> str:
        lines = [
            "LEVI × L.W.P. — Phase map (local-first + optional cloud wings)",
            f"Current: Phase {current_phase().id} — {current_phase().name} [{current_phase().status}]",
            "",
        ]
        for p in self.all():
            marker = "▶" if p.id == current_phase().id else " "
