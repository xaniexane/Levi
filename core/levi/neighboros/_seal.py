"""Veil-lineage sealing, NeighborOS scope.
...
keystream XOR, Encrypt-then-MAC. Separate key and home from every
other package (``<LEVI_HOME>/neighboros/``) — a NeighborOS receipt
can never be opened with an academy key.
...
def pkg_dir(home: Optional[Path] = None) -> Path:
    ...
    d = (Path(home) if home is not None else Path(base).expanduser()) / "neighboros"
