...
class FactorScore:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.value, (int, float)) or isinstance(self.value, bool):
            raise ValueError(f"factor {self.name!r}: value must be numeric, got {self.value!r}")
...
def validate_weights(weights: Optional[Mapping[str, float]]) -> Dict[str, float]:
    ...
    if set(w) != set(FACTORS):
        raise ValueError(
            f"weights must cover exactly {FACTORS}; got {sorted(w)}"
        )
...
def _coerce_factor(name: str, raw: FactorInput) -> FactorScore:
    ...
    if isinstance(raw, Mapping):
        return FactorScore(name=name, value=raw["value"], basis=raw["basis"])
    value, basis = raw  # tuple / pair
...
def validate_factors(factors: Mapping[str, FactorInput]) -> List[FactorScore]:
    ...
    if set(factors) != set(FACTORS):
        ...
        raise ValueError(f"factors must be exactly {FACTORS}; missing={missing} extra={extra}")
    return [_coerce_factor(name, factors[name]) for name in FACTORS]


def composite_score(
    factors: List[FactorScore], weights: Mapping[str, float]
) -> float:
    """Deterministic weighted mean of factor values, rounded to 2 dp."""
    total = sum(f.value * float(weights[f.name]) for f in factors)
