...
def ex_coverage_map(entry, lesson, research):
    ...
    return _grade(art, [
        ...
        ("every mapping has a rationale", all(_nonempty(m["rationale"]) for m in mappings)),
...
def ex_detection_sketch(entry, lesson, research):
    ...
    sigma = {
        ...
        "detection": {
            "selection": {f"EventID": 4688,
                          "ProcessName|contains": terms},
            "condition": "selection",
        },
        "note": ("Sketch only: a production rule needs baselining, "
                 "allowlisting, and CI testing before deployment."),
    }
    art = {"sigma": sigma}
    return _grade(art, [
        ("has title", _nonempty(sigma["title"], 10)),
        ("has logsource", isinstance(sigma["logsource"], dict) and bool(sigma["logsource"])),
        ("has detection", isinstance(sigma["detection"], dict) and bool(sigma["detection"])),
...
def ex_hunt_hypothesis(entry, lesson, research):
    ...
    art = {
        ...
        "scope": ("One business unit, 14 days of endpoint and identity "
                  "telemetry, read-only queries, no production changes."),
        "falsification": ("The hypothesis is disproved if the scoped "
    ...
    return _grade(art, [
        ("hypothesis is if/then", "if " in art["hypothesis"].lower() and "then" in art["hypothesis"].lower()),
...
def ex_triage(entry, lesson, research):
    ...
    queue = [
        {"alert": alerts[i] if i < len(alerts) else f"alert-{i}",
         "priority": rank + 1,
         "rationale": ("Ranked by severity x fidelity x asset value, per the "
    ...
    return _grade(art, [
        ...
        ("every item has a rationale", all(_nonempty(q["rationale"]) for q in queue)),
    ])
...
def ex_hardening_check(entry, lesson, research):
    checks = [
        {"check": obj,
         "verify": ("Verify with an automated check or audit query, not by "
    ...
    return _grade(art, [
        ...
        ("every check names verification", all(_nonempty(c["verify"]) for c in checks)),
    ])
...
def ex_intel_brief(entry, lesson, research):
    ...
    art = {
        ...
        "confidence": ("Medium: grounded in public sources and the session's "
                       "own material; corroborate before operational use."),
    }
    return _grade(art, [
        ("bottom line present", _nonempty(art["bottom_line"])),
        ("evidence listed", isinstance(art["evidence"], list) and len(art["evidence"]) >= 1),
...
def ex_platform_teardown(entry, lesson, research):
    art = {
        ...
        "performance": ("How it performs: judged by the outcomes users feel — "
        ...
        "adapt": ("Adapt to LEVI's constraints: local-first, stdlib-only, "
                  "free core forever. The pattern survives; the implementation changes."),
        "differ": ("Deliberately differ where platforms manipulate: separate "
...
def ex_ux_deconstruction(entry, lesson, research):
    art = {
        ...
        "what_works": ("What works: clarity, progressive disclosure, and "
                       "user-controlled detail depth."),
        "manipulation_risk": ("Risk: urgency cues, gamified streaks, and "
                              "color-coded pressure that push users to act "
                              "against their interests."),
        "levi_rule": ("LEVI's rule: separate information from pressure. Show "
...
def ex_comparison(entry, lesson, research):
    art = {
        "dimensions": ["operating model", "performance", "failure modes", "fit for LEVI"],
        "options": [
            {"option": "Adopt", "note": "Take the proven pattern as-is."},
            {"option": "Adapt", "note": "Reshape for local-first, stdlib-only, free core."},
            {"option": "Differ", "note": "Deliberately diverge where platforms manipulate or centralize."},
        ],
        "verdict": (f"For '{entry['title']}': adopt the mechanics, adapt the "
...
def _probe_finance(tmp: str) -> dict:
    ...
    bars = [Bar(date=f"2026-08-{d:02d}", open=100 + d, high=102 + d,
                low=99 + d, close=101 + d, volume=1000.0) for d in range(1, 31)]
    sig = score_bars("SYNTH", bars)
    return {"ok": True, "detail": f"score_bars on 30 synthetic bars -> action={sig.action}"}


def _probe_demand(tmp: str) -> dict:
    from levi.demand.pulse import DemandPulse
    pulse = DemandPulse(path=Path(tmp) / "demand.json")
    sig = pulse.scan_seed("academy drill: operators need faster triage queues", segment="security")
    return {"ok": True, "detail": f"scan_seed registered signal id={sig.id} (HYPOTHESIS-labeled, isolated path)"}


def _probe_memory(tmp: str) -> dict:
    from levi.memory.store import MemoryStore
    store = MemoryStore(data_dir=Path(tmp) / "memory")
    return {"ok": True, "detail": f"MemoryStore opened at isolated dir; entries={len(store._entries)}"}
...
def _probe_news(tmp: str) -> dict:
    ...
    return {"ok": True, "detail": f"news corpus files visible: {n} (read-only count, no refresh)"}
...
def _probe_schedule(tmp: str) -> dict:
    spec = {"kind": "interval", "every": "6h", "id": "academy-drill",
            "note": "spec constructed, never installed by the drill"}
    return {"ok": True, "detail": f"cron spec constructed: {spec['every']} (not installed)"}
...
def _probe_agent(tmp: str) -> dict:
    ...
    return {"ok": True, "detail": f"agent tools module exposes {len(names)} names (read-only dir)"}
...
def _select_probes(entry) -> list[str]:
    ...
    picked = [name for key, name in
              [("finance", "finance"), ("demand", "demand"), ("news", "news"),
               ("memory", "memory"), ("skill", "skill"), ("schedul", "schedule"),
               ("agent", "agent"), ("secur", "security")]
              if key in hay]
...
def ex_module_drill(entry, lesson, research):
    ...
    with tempfile.TemporaryDirectory(prefix="academy-drill-") as tmp:
        for name in _select_probes(entry):
            ...
            try:
                r = fn(tmp)
                results.append({"name": name, "ok": bool(r.get("ok")), "detail": r.get("detail", "")})
            except Exception as exc:  # drills never fail the session
                results.append({"name": name, "ok": False, "detail": f"skipped: {type(exc).__name__}"})
    art = {"probes": results}
    return _grade(art, [
...
def ex_orchestration_drill(entry, lesson, research):
    ...
    art = {"chain": chain,
           "receipt": (f"Orchestration drill chained {len(chain)} module "
...
def ex_design_review(entry, lesson, research):
    art = {
        ...
        "tradeoffs": ("Speed versus safety; cost versus control. LEVI's "
                      "standing trade: free core forever, value in the "
                      "managed layers."),
        "recommendation": ("Recommend the option that keeps guardrails green "
...
def ex_sparring_scenario(entry, lesson, research, interleave=None):
    ...
    art = {
        ...
        "calls": ("Calls made under pressure: prioritize by evidence, keep "
                  "the decision log, escalate what you cannot verify."
                  + (f" Interleaved cross-track concepts applied: "
                     f"{'; '.join(woven)}." if woven else "")),
        "debrief": ("Debrief: what worked, what broke, and the one change "
...
def ex_tabletop(entry, lesson, research, interleave=None):
    ...
    art = {
        ...
        "decisions": [f"Decision {i+1}: {o}" for i, o in enumerate(entry["objectives"])],
        "interleaved_concepts": [c["name"] for c in interleave],
        "action_items": [f"Action: operationalize '{o}' with an owner and a date."
...
def ex_capstone(entry, lesson, research):
    art = {
        ...
        "plan": "; ".join(entry["objectives"]),
        "execution": ("Executed across all three tracks: defensive method, "
                      "platform awareness, LEVI systems operation — with "
                      "receipts at every step."),
        "review": ("Honest review: what held, what broke, and what the next "
...
def _fallback_track_grade(track: str) -> tuple[float, list[dict]]:
    ...
    """
    import os
    dd = Path(os.environ.get("LEVI_ACADEMY_DIR",
                             Path.home() / ".levi" / "academy"))
    p = dd / "progress.json"
    scores: list[float] = []
    if p.exists():
        try:
            ...
            for s in prog.get("sessions", {}).values():
                if (s.get("track") == track
                        and isinstance(s.get("mastery_score"), (int, float))):
                    scores.append(float(s["mastery_score"]))
        except Exception:
            pass
    mean = sum(scores) / len(scores) if scores else 0.0
    return mean, [{"concept_id": f"{track}-fallback",
                   "name": f"{track}: completed-session mastery fallback",
                   "day": 0, "kind": "fallback", "score": round(mean, 3),
                   "note": "concept registry empty; graded from "
                           f"{len(scores)} completed session(s)' mastery"}]


def ex_graduation(entry, lesson, research, syllabus, journal_points,
                  interleave=None):
    ...
    for track in ("A", "B", "C"):
        ...
        if weak:
            for c in weak:
                ...
                tested.append({"concept_id": c["id"], "name": c["name"],
                               "day": c["day"], "kind": c["kind"],
                               "score": drill["score"]})
    ...
    focus = [t["name"] for t in per_track[weakest_track]["tested"]
             if isinstance(t.get("score"), float) and t["score"] < 0.7][:4]
    verdict = ("GRADUATED" if overall >= 0.6
               and all(v["score"] >= 0.5 for v in per_track.values())
               else "NOT YET")
    art = {"per_track": per_track, "overall": overall,
           "weakest_track": weakest_track,
           "post_graduation_focus": focus or
           [t.get("name", "") for t in per_track[weakest_track]["tested"][:4]],
           "verdict": verdict,
...
def run_exercise(exercise_type, entry, lesson, research,
                 syllabus=None, journal_points=None, interleave=None):
    if exercise_type == "graduation":
        return ex_graduation(entry, lesson, research, syllabus or {},
                             journal_points or [], interleave=interleave)
