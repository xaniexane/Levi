...
def cmd_define(args) -> int:
    try:
        c = _store().define(args.name, unit=args.unit, target=args.target,
                            per=args.per,
        ...
        return 1
    print("commitment '%s': %g %s per %s%s%s" % (
        c["name"], c["target"], c["unit"], c["per"],
        " (rest: %d/wk)" % c["rest_days_per_week"] if c["rest_days_per_week"] else "",
        " (mulligans: %d/mo)" % c["mulligans_per_month"] if c["mulligans_per_month"] else ""))
...
def _show_status(st: CommitmentStore, name: str) -> int:
    ...
    print("  %d/%d periods hit · longest run %d · target %g %s/%s" % (
        s["hits"], s["periods"], s["longest"], s["target"],
        s["unit"], s["per"]))
    if s["missed"]:
...
def cmd_status(args) -> int:
    ...
    if not names:
        print("no commitments — define one with: "
              "python -m levi.commitments define NAME")
        return 0
    rc = 0
    for n in names:
...
def cmd_pause(args) -> int:
    try:
        ...
        print("commitments: %s" % exc, file=sys.stderr)
        return 1
    print("paused '%s' — streak is held, not broken. Come back when you want."
          % args.name)
    return 0
...
def cmd_delete(args) -> int:
    try:
        ...
        print("commitments: %s" % exc, file=sys.stderr)
        return 1
    print("deleted '%s'. No penalty, no guilt — it was yours to end."
          % args.name)
    return 0
...
def main(argv=None) -> int:
    ...
    p.add_argument("--rest-days", type=int, default=0,
                   help="rest days per week (Sat/Sun first)")
    p.add_argument("--mulligans", type=int, default=0,
                   help="forgiven misses per month")
