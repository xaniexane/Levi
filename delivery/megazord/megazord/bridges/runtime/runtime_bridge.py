...
class MemoryStore:
    ...
    def store(
        ...
        e = MemoryEntry(
            id=str(uuid.uuid4())[:12], key=key, value=value, tags=tags or []
        )
    ...
    def stats(self) -> Dict[str, Any]:
        return {
            ...
            "most_accessed": sorted(
                self._entries.values(), key=lambda e: e.access_count, reverse=True
            )[:5],
        }
    ...
    def export_json(self) -> str:
        return json.dumps(
            [
                {"key": e.key, "value": e.value, "tags": e.tags, "ts": e.ts}
                for e in self._entries.values()
            ],
            indent=2,
        )
...
class RuntimeBrain:
    ...
    def context_window(self, n: int = 20) -> List[MemoryEntry]:
        ...
        return sorted(
            self.store.all(), key=lambda e: (e.ts, -e.access_count), reverse=True
        )[:n]
