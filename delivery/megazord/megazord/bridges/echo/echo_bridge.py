...
class EchoBridge:
    ...
    def generate_blueprint(self, prompt: str, options: Optional[Dict[str, Any]] = None) -> Blueprint:
        ...
        if "job"     in p_lower: app_name = "Hybrid Job Agent"
        if "social"  in p_lower: app_name = "Social Network App"
        if "market"  in p_lower: app_name = "Marketplace"
        if "chat"    in p_lower: app_name = "Chat App"
        ...
        spec = {
            "app_name":    app_name,
            "version":     "1.0.0",
            "models":      [{"name": m, "fields": ["id", "created_at", "status"]} for m in models],
            "actions":     [{"name": a, "type": "function", "params": []} for a in actions],
            "views":       [{"name": v, "type": "list", "data": models[0] if models else "item",
                             "fields": ["name", "status"], "actions": []} for v in views],
            "workflows":   workflows,
    ...
    def _infer_components(self, prompt: str, app_name: str):
        p = prompt.lower()
        if "job" in p:
            return (["job","application"], ["search_jobs","prefill_application","hitl_approve"],
                    ["job_list","hitl_queue","dashboard"])
        if "social" in p:
            return (["post","comment","profile"], ["create_post","comment","follow","block"],
                    ["feed","profile","notifications"])
        if "market" in p:
            return (["product","order","user"], ["list_product","buy","rate","refund"],
    ...
    def _infer_workflows(self, prompt: str, actions: List[str]) -> List[Dict[str, Any]]:
        ...
        if "daily" in p or "every day" in p:
            return [{"name": "daily_flow", "trigger": "daily_schedule",
                     "steps": [{"action": actions[0] if actions else "process_item", "with": {}}]}]
        return []

    def _infer_automations(self, prompt: str, actions: List[str]) -> List[Dict[str, Any]]:
        p = prompt.lower()
        if "notify" in p or "alert" in p:
            return [{"name": "notify_on_change", "trigger": "status_changed",
                     "conditions": [], "steps": [{"action": "send_notification", "with": {"message": "Status changed."}}]}]
        return []
    ...
    def build_lwp_action(self, blueprint: Blueprint) -> Dict[str, Any]:
        ...
        return self.levi.build_action(
            ...
            priority="normal",
            soul={"joy": 0.7, "trust": 0.6, "fear": 0.1, "surprise": 0.8, "sadness": 0.0},
        )
