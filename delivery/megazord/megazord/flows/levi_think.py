...
def _observe(prompt: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intent_hint = (
        "security"  if any(w in prompt_lower for w in ["scan","threat","secure","block","protect"]) else
        "build"    if any(w in prompt_lower for w in ["make","create","design","build","generate","compile"]) else
        "orchestrate" if any(w in prompt_lower for w in ["run","execute","orchestrate","schedule","automate"]) else
        "diagnose" if any(w in prompt_lower for w in ["check","diagnose","status","health","report"]) else
        "unknown"
    )
    return {
...
def _interpret(observation: Dict[str, Any], persona: Optional[Persona] = None) -> Dict[str, Any]:
    ...
    soul = persona.soul.to_dict() if persona else {"joy":0,"trust":0,"fear":0,"surprise":0,"sadness":0}
    return {
...
class LeviThinkFlow(Flow):
    ...
    def __init__(self, prompt: str, persona: Optional[Persona] = None, context: Optional[Dict[str, Any]] = None):
        super().__init__(
            id="levi_think_v1",
            name="LeviThink",
