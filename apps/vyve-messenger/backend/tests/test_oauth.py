...
def _authorize(oauth_mod, oauth_client, redirect_uri="http://localhost:8081/callback",
               username="chauncey", password="changeme", **extra):
...
def _exchange_code(oauth_client, code, verifier,
                   redirect_uri="http://localhost:8081/callback"):
    return oauth_client.post("/oauth/token", data={
...
def test_pkce_flow_issues_rs256_tokens(oauth_mod, oauth_client):
    ...
    me = oauth_client.get("/oauth/userinfo",
...
def test_authorization_code_reuse_revokes(oauth_mod, oauth_client):
    ...
    rotated = oauth_client.post("/oauth/token", data={
...
def test_refresh_token_rotation(oauth_mod, oauth_client):
    ...
    second = oauth_client.post("/oauth/token", data={
        "grant_type": "refresh_token",
    ...
    stale = oauth_client.post("/oauth/token", data={
        "grant_type": "refresh_token",
        "refresh_token": first["refresh_token"],
        "client_id": "vyve-messenger",
        "client_secret": TEST_CLIENT_SECRET,
...
def test_startup_fails_without_client_secret():
    ...
    code = ("import sys; sys.path.insert(0, %r); "
            "import oauth.server" % BACKEND_DIR)
...
def test_device_registration_requires_valid_token(oauth_mod, oauth_client):
    ...
    body = {"device_id": "d1", "device_name": "phone", "device_type": "android",
            "signing_key": "sig", "encryption_key": "enc"}
    ...
    bad = oauth_client.post("/me/devices", json=body,
                            headers=_authz(tamper_payload(token)))
