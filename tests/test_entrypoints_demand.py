...
def test_scan_and_five_factor(monkeypatch, tmp_path, capsys):
    ...
    rc = main([
        "--scan", "I need help scheduling shifts",
        "--title", "Shift planner",
        "--five-factor",
        "--ff-demand", "70", "--ff-market", "60", "--ff-gap", "80",
        "--ff-velocity", "50", "--ff-feasibility", "90",
        "--ff-basis", "test estimates",
    ])
    assert rc == 0
...
def test_five_factor_without_basis_fails_closed(monkeypatch, tmp_path, capsys):
    _herm(monkeypatch, tmp_path)
    rc = main([
        "--scan", "need shifts help", "--title", "X", "--five-factor",
        "--ff-demand", "70", "--ff-market", "60", "--ff-gap", "80",
        "--ff-velocity", "50", "--ff-feasibility", "90",
    ])
    assert rc == 2
