...
def test_arrival_banner_legible():
    ...
    assert banner == (
        "Second mind 'verifier-op' joined — high-stakes turn (stakes assessed high)."
    )
...
def test_arrival_banner_on_forced_fork():
    reg = _registry("VERIFIED")
    turn = run_twin_turn(reg, "chat", _user("hi"), [], {"force_twin": True}, _config())
    assert turn.arrival is not None
...
def test_ratified_default_mode_is_on_demand():
    ...
    pair_off = resolve_twin(
        "chat",
        {
            "twins": {
                "mode": "off",
                "seats": {"chat": {"primary": "a", "verifier": "b"}},
            }
        },
    )
