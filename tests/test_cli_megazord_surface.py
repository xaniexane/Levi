...
def test_methods_list_names_shelves(capsys):
    climain.cmd_methods(argparse.Namespace(methods_action="list",
                                           methods_name=None))
...
def test_methods_show_prints_doc(capsys):
    climain.cmd_methods(argparse.Namespace(methods_action="show",
                                           methods_name="loci"))
...
def test_revival_list_names_shelves(capsys):
    climain.cmd_revival(argparse.Namespace(revival_action="list",
                                           revival_name=None))
    out = capsys.readouterr().out
...
def test_atlas_export_to_file(monkeypatch, tmp_path):
    ...
    climain.cmd_atlas(argparse.Namespace(atlas_action="export",
                                         out=str(out)))
    assert out.is_file()
...
def test_workflow_list_shows_workflows(monkeypatch, tmp_path, capsys):
    _herm_home(monkeypatch, tmp_path)
    climain.cmd_workflow(argparse.Namespace(workflow_action="list",
                                            name=None))
    out = capsys.readouterr().out
...
def test_workflow_run_without_name_exits_2(monkeypatch, tmp_path, capsys):
    _herm_home(monkeypatch, tmp_path)
    with pytest.raises(SystemExit) as exc:
        climain.cmd_workflow(argparse.Namespace(workflow_action="run",
                                                name=None))
    assert exc.value.code == 2
...
def test_warehouse_list_browse_inventory_pull(monkeypatch, tmp_path, capsys):
    _herm_home(monkeypatch, tmp_path)
    climain.cmd_warehouse(argparse.Namespace(warehouse_action="list",
                                             name=None, item=None, limit=20))
    assert "LEVI warehouses" in capsys.readouterr().out

    climain.cmd_warehouse(argparse.Namespace(warehouse_action="browse",
                                             name="methods", item=None,
                                             limit=20))
    out = capsys.readouterr().out
    assert "Methods Warehouse" in out

    climain.cmd_warehouse(argparse.Namespace(warehouse_action="inventory",
                                             name="methods", item=None,
                                             limit=3))
    out = capsys.readouterr().out
    assert "methods.ach" in out

    climain.cmd_warehouse(argparse.Namespace(warehouse_action="pull",
                                             name="methods",
                                             item="methods.loci", limit=20))
    out = capsys.readouterr().out
...
def test_bloodstream_turn_dispatches(monkeypatch, capsys):
    ...
    climain.cmd_bloodstream(argparse.Namespace(bloodstream_action="turn",
                                              text=["hello", "levi"]))
...
def test_bloodstream_bus_test_reports_stages(monkeypatch, capsys):
    ...
    monkeypatch.setattr(_turn, "run_turn",
                        lambda text, ctx=None: _fake_turn_result())
    climain.cmd_bloodstream(argparse.Namespace(bloodstream_action="bus-test",
                                              text=[]))
    out = capsys.readouterr().out
...
def test_factory_status_stages_with_mocked_readers(monkeypatch, tmp_path,
                                                   capsys):
    _herm_home(monkeypatch, tmp_path)
    from levi.perpetual import hunt as _hunt
    ...
    wave = SimpleNamespace(id="wave-001", theme_id="retired-software",
                           completed_at="2026-09-15T00:00:00",
                           findings_count=7, status="completed")
    state = SimpleNamespace(waves=[wave], next_due="2026-09-16T05:00:00")
    monkeypatch.setattr(_hunt, "load_state", lambda *a, **k: state)
    monkeypatch.setattr(_hunt, "read_build_queue",
                        lambda *a, **k: [{"wave": "wave-001"}] * 3)
...
def test_lifepack_preview_is_read_only(monkeypatch, tmp_path, capsys):
    ...
    climain._cmd_lifepack_preview(argparse.Namespace(file=str(path),
                                                    lifepack_action="preview"))
    out = capsys.readouterr().out
...
def test_lifepack_preview_missing_file_exits_1(monkeypatch, tmp_path, capsys):
    ...
    with pytest.raises(SystemExit) as exc:
        climain._cmd_lifepack_preview(
            argparse.Namespace(file=str(tmp_path / "nope.json"),
                               lifepack_action="preview"))
    assert exc.value.code == 1
...
def test_daemon_services_lists_catalog(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("HOME", str(tmp_path))
    climain.cmd_daemon_services(argparse.Namespace(action="services",
                                                  target=None))
    out = capsys.readouterr().out
