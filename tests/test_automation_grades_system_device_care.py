"""Grade-manifest tests for the System & Device Care minion batch.

Batch slug: system_device_care. All 42 minions are expected at grade
...
MANIFEST_PATH = Path("core/levi/automation/grade_data/system_device_care.json")
...
def test_manifest_loads_and_verifies():
    data = _load()
    assert data["record_count"] == len(data["records"]) == EXPECTED_COUNT
...
def test_record_ids_match_batch_exactly():
    data = _load()
    expected = {m.id for m in by_category(CATEGORY)}
